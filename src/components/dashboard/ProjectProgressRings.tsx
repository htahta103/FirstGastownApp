...
function Ring({
  ...
  return (
    <Link
      ...
      className="group flex flex-col items-center gap-2 rounded-xl p-3 transition-colors hover:bg-white/[0.04]"
    >
      <div className="relative size-[5.5rem]">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-sm font-semibold tabular-nums text-gray-100">{pct}%</span>
        </div>
      </div>
      <span className="max-w-[6.5rem] truncate text-center text-xs font-medium text-gray-400 group-hover:text-violet-300">
        {label}
      </span>
...
export function ProjectProgressRings({ rows }: { rows: Row[] }) {
  if (rows.length === 0) {
    return (
      <Card variant="subtle" className="border-dashed border-white/10">
  ...
  return (
    <Card variant="default" hoverable={false} className="border-white/[0.08]">
      <CardHeader>
